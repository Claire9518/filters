    ...
    def generate(self, isLite=False):
        try:
            ...
            with open(fileName, 'a') as f:
                f.write("#\n")
                if isLite:
                    f.write("#!name=AdBlock Surge Lite-Claire\n")
                    f.write("#!desc=适用于 Surge 的去广告合并规则，每 12 个小时更新一次。规则源：%s。Lite 版仅针对国内域名拦截。\n"%(self.sourceRule))
                else:
                    f.write("#!name=AdBlock Surge-Claire\n")
                    f.write("#!desc=适用于 Surge 的去广告合并规则，每 12 个小时更新一次。规则源：%s。\n"%(self.sourceRule))
                f.write("#!homepage=%s\n"%(self.homepage))
